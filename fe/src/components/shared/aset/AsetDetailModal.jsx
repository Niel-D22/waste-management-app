 ...
 */
function AsetDetailModal({ data, onClose, footerActions }) {
  if (!data) return null;
      ...
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto bg-[#F8FAFC] p-6">
          ...
          <div className="flex flex-col items-start gap-5 sm:flex-row">
            <div className="flex size-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
            ...
            <div className="flex-1">
              <span className="mb-2 inline-block rounded bg-emerald-50 px-2 py-1 text-xs font-bold tracking-wider text-emerald-600 uppercase">
                {data.kategori_aset?.nama || "—"}
              </span>
              ...
              <p className="text-sm leading-relaxed whitespace-pre-wrap text-gray-500">
                {data.deskripsi_aset || "Tidak ada deskripsi."}
              </p>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 font-medium text-gray-800">
                      {data.status_aktif ? "Beroperasi" : "Tidak Aktif"}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {/* Semua Foto */}
          {data.pictures_urls && data.pictures_urls.length > 1 && (
            <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              <h4 className="mb-4 text-sm font-bold text-gray-900">
                Galeri Foto ({data.pictures_urls.length})
              </h4>
              <div className="flex gap-3 overflow-x-auto pb-2">
                {data.pictures_urls.map((url, i) => (
                  <img
                    key={i}
                    src={url}
                    alt={`Foto Aset ${i + 1}`}
                    className="h-24 min-w-24 rounded-lg border border-gray-200 bg-gray-100 object-cover"
                  />
                ))}
              </div>
            </div>
...
export default AsetDetailModal;
