...
const markerIcon = new L.divIcon({
  ...
  html: `
    <div class="relative flex items-center justify-center size-10">
      ...
      <div class="relative size-6 bg-red-500 border-[3px] border-white rounded-full shadow-lg z-10"></div>
    </div>
  `,
  iconSize: [40, 40],
  iconAnchor: [20, 20],
...
// ─── Validasi ─────────────────────────────────────────────────────
const RULES = {
  alamat_lokasi: { minLength: 10, maxLength: 300 },
};
...
const inputClass = (touched, error) => {
  const base =
    "w-full rounded border px-4 py-2.5 text-sm focus:outline-none transition-colors resize-none";
...
const reverseGeocode = async (lat, lng) => {
  try {
    ...
    if (data && data.address) {
      ...
      return { kabupaten_kota: kabKota, alamat_lokasi: alamat };
    }
    return null;
  } catch {
    return null;
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      return {
        ...
        alamat_lokasi: result.display_name || query,
      };
    }
    return null;
  } catch {
  ...
  const handleMapClick = async (lat, lng) => {
    ...
    if (result) {
      setFormData((prev) => ({
        ...
        alamat_lokasi: result.alamat_lokasi,
      }));
    }
    setGeocoding(false);
  };
    ...
    <div className="animate-in fade-in space-y-5 duration-300">
        ...
        <form onSubmit={handleSearchAddress} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded border border-gray-300 py-2.5 pr-4 pl-9 text-sm focus:border-[#1e1f78] focus:outline-none"
            />
          </div>
          <button
            type="submit"
            disabled={geocoding}
            className="shrink-0 rounded bg-[#1e1f78] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#16175e] disabled:opacity-50"
          >
            {geocoding ? "..." : "Cari"}
          </button>
        </form>
        ...
        <div className="flex items-center justify-between border-t border-gray-100 bg-gray-50 px-4 py-2">
          <p className="text-[11px] text-gray-500">
            {hasMarker ? (
              ...
              "Klik pada peta atau gunakan GPS untuk menandai lokasi"
            )}
          </p>
          {geocoding && (
      ...
      <div>
        ...
        <textarea
          ...
          rows="3"
          maxLength={300}
          placeholder="Terisi otomatis dari peta, dan tambahkan patokan seperti: Depan SDN 01 Tikala, dekat tiang listrik..."
          className={inputClass(touched.alamat_lokasi, errors.alamat_lokasi)}
        />
        {touched.alamat_lokasi && errors.alamat_lokasi ? (
          <ErrorMsg msg={errors.alamat_lokasi} />
        ) : (
          <p className="mt-1 text-right text-[11px] text-gray-400">
            {(formData.alamat_lokasi || "").length}/300
          </p>
        )}
      </div>
    </div>
